#' Create a Co-occurrence Matrix Table of Codes
#'
#' @param excerpts A data frame with one column named `media_title` identifying
#'   transcripts, and code columns beginning with `"c_"` (logical or numeric).
#' @param min_bold Threshold for bolding co-occurrence values in the table.
#' @param scale Whether to display raw \code{"count"} or \code{"proportion"}
#'   (normalized by the number of transcripts where each code appears).
#'   Defaults to "count".
#' @param output Type of output: "kable" (default), "tibble", or "data.frame".
#'
#' @return Depending on `output`, either:
#'   - a `knitr::kable` HTML table,
#'   - a tibble,
#'   - or a base R data frame.
#' @export
