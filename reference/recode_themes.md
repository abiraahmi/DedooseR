...
- data:
  ...
  output from clean_data() or a dataset read from a .dta file)
...
- recodes:
  ...
  example: list(c_help = c("c_support", "c_assist"), c_stress =
  c("c_anxiety", "c_pressure"))
...
recodes, assigns user-specified or default labels, removes the original
source variables (unless one overlaps with a new variable name), and
builds a codebook summarizing the recoded dataset.
