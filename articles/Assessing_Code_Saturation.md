# 3. Assessing Code Saturation

Assessing saturation often comes down to two practical questions:
**Which codes show up often enough?** and **Do they stretch across a
wide enough slice of transcripts or media objects?** The
[`set_saturation()`](https://abiraahmi.github.io/DedooseR/reference/set_saturation.md)
and
[`compare_saturation()`](https://abiraahmi.github.io/DedooseR/reference/compare_saturation.md)
helpers turn those questions into repeatable filters and visuals. This
vignette builds on the same mock excerpts used in the earlier articles
so you can knit everything in order or cherry-pick what you need.

If you knitted the *Cleaning, Merging, and Exploring Qualitative Data*
or *Summarizing Codes* vignettes just before this one, you already have
`recoded$data_recode` and `recoded$codebook_recode` in your environment.
The chunk below recreates them so this article is self-contained—feel
free to skip it when reusing objects from the prior tutorial.
...
[`set_saturation()`](https://abiraahmi.github.io/DedooseR/reference/set_saturation.md)
uses the output of `create_code_summary` to filter and visualiz codes
that meet minimum appearance targets. Here we look for at least eight
excerpts and presence in 40% of media titles (eight of the 20
transcripts). The dual-axis plot keeps counts and coverage together.
...
#>   <chr>                                              <int>             <dbl>
#> 1 academic skills                                       20              1   
#> 2 Emotional growth: identification and/or regulation    15              0.75
#> 3 self-esteem                                           15              0.75
#> 4 growth mindset                                        12              0.6 
#> 5 internalizing behavior                                12              0.6 
#> 6 inhibitory control                                    10              0.5 
#> 7 social skills                                         10              0.5 
#> 8 self-expression                                        9              0.45
```
...
![](Assessing_Code_Saturation_files/figure-html/unnamed-chunk-3-1.png)

[`compare_saturation()`](https://abiraahmi.github.io/DedooseR/reference/compare_saturation.md)
builds on the same summary table to check multiple threshold sets at
once - useful when you want a strict bar versus a more liberal bar. Each
threshold needs a minimum excerpt count and a minimum proportion of
media titles.
...
#>    <chr>           <int>          <int>             <dbl> <lgl>                 
#>  1 academic skills    20             20              1    TRUE                  
#>  2 Emotional grow…    15             15              0.75 TRUE                  
#>  3 externalizing …     6              6              0.3  FALSE                 
#>  4 growth mindset     12             12              0.6  TRUE                  
#>  5 inhibitory con…    10             10              0.5  TRUE                  
#>  6 internalizing …    12             12              0.6  TRUE                  
#>  7 motor skills        5              5              0.25 FALSE                 
#>  8 self-esteem        15             15              0.75 TRUE                  
#>  9 self-expression     9              9              0.45 TRUE                  
#> 10 social skills      10             10              0.5  TRUE                  
#> # ℹ abbreviated name: ¹​`Liberal (>=8 excerpts, >=40% transcripts)`
#> # ℹ 1 more variable: `Strict (>=12 excerpts, >=60% transcripts)` <lgl>
```

And here’s the faceted view that spotlights which codes meet each target
level.
...
![](Assessing_Code_Saturation_files/figure-html/unnamed-chunk-6-1.png)
