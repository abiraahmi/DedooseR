...
Co-occurrence analysis helps you see which codes travel together within
the same transcript or media title. The
[`cooccur()`](https://abiraahmi.github.io/DedooseR/reference/cooccur.md)
helper builds both a matrix and a network plot from the recoded tibble
we have been using throughout the documentation set.

If you just knit the earlier vignettes, `data` and `codebook` may still
be in your environment. The chunk below rebuilds them so this article
stands alone.
...
By default
[`cooccur()`](https://abiraahmi.github.io/DedooseR/reference/cooccur.md)
returns a formatted HTML table, but for easy inspection we will request
a tibble and keep a moderately inclusive edge threshold in the plot.
...
#> # A tibble: 10 × 11
#>    code    `motor skills` `social skills` `self-expression` `inhibitory control`
#>    <chr>            <dbl>           <dbl>             <dbl>                <dbl>
#>  1 motor …              5               2                 2                    4
#>  2 social…              2              10                 3                    5
#>  3 self-e…              2               3                 9                    4
...
#>  9 growth…              4               6                 7                    8
#> 10 Emotio…              3               7                 5                    9
#> # ℹ 6 more variables: `internalizing behavior` <dbl>,
#> #   `externalizing behavior` <dbl>, `academic skills` <dbl>,
#> #   `self-esteem` <dbl>, `growth mindset` <dbl>,
#> #   `Emotional growth: identification and/or regulation` <dbl>
```
...
![](Cooccurence_files/figure-html/unnamed-chunk-4-1.png)

Codes such as academic skills, emotional growth, and self-esteem form a
dense cluster - unsurprising given how often they appear together across
the 20 media titles.

In the proportion table you can see, for example, that emotional growth
appears in 80% of transcripts where self-esteem is coded, while
externalizing behavior rarely co-occurs with academic skills.
